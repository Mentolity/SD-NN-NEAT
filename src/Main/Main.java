...
/*
		...
		while(n <=5) {
			neat.runGeneration(); n++;}
		System.out.println("COMPLETED: " + neat.getBestSpeciesFitness());
		//sav.saveNet(neat.getBestNetwork(), null);
		sav.savePop(neat.getPop(), new File("./src/res/sv2.txt"));

*/
		//NEATNetwork net2 = ser.loadNet(null);
		//sav.saveNet(net2, new File("./src/res/sv3.txt"));
/*
		//for comparing saved networks, testing
		SwingUtilities.invokeLater(new Runnable() {
			public void run(){
				try{
					//GNeuralNetwork frame = new GNeuralNetwork(neat.getBestNetwork());
					GNeuralNetwork frame = new GNeuralNetwork(net2);
